...
class PandaSpider(metaclass=SpiderMeta):
    start_url = 'http://www.xiongmaodaili.com/xiongmao-web/freeip/list'

    def gets(self, page_total=2):
        ans = []
        html = get_page(self.start_url, True)
        time.sleep(1)
        if html:
...
class YundailiSpider(metaclass=SpiderMeta):
    start_url = 'http://www.ip3366.net/?stype=1&page={}'

    def gets(self, page_total=2):
        urls = [self.start_url.format(page) for page in range(self._counter, page_total + self._counter)]
        self.increment(page_total)
        ans = []
        for url in urls:
...
class KxdailiSpider(metaclass=SpiderMeta):
...
class Data5USpider(metaclass=SpiderMeta):
    start_url = 'http://www.data5u.com/free/{}/index.shtml'

    def gets(self, page_total=2):
        urls = [self.start_url.format(type) for type in ['gngn', 'gnpt']]
        ans = []
        for url in urls:
            html = get_page(url, True)
            time.sleep(1)
